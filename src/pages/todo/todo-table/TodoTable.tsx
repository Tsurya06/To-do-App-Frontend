// import {
//   Row,
//   Col,
//   Table,
//   Button,
//   message,
//   Pagination,
//   Modal,
//   DatePicker,
// } from "antd";
// import {
//   RootState,
//   useAppDispatch,
//   useAppSelector,
// } from "../../../store/store";
// import { ColumnsType } from "antd/es/table";
// import { TaskType } from "../../../types/apiResponseType";
// import {
//   DeleteTaskByIdThunk,
//   EditTaskThunk,
//   GetTaskList,
// } from "../../../store/features/task/TaskThunk";
// import { useEffect, useState } from "react";
// import {
//   DeleteOutlined,
//   EditOutlined,
// } from "@ant-design/icons";
// import { useSearchParams } from "react-router-dom";
// import EditTodoModal from "./modals/EditTodoModal";
// export type FilterTodoObjectType = {
//   pageSize: number;
//   currentPage: number;
//   date?: string | undefined;
// };
// export type SearchParamsType = {
//   pageSize: string;
//   currentPage: string;
//   date?: string | undefined;
// };
// export default function TodoTable() {
//   const todos = useAppSelector((state: RootState) => state.taskReducer);
//   const [modalOpen, setModalOpen] = useState<boolean>(false);
//   const [deletePromptModalOpen, setDeletePromptModalOpen] = useState<boolean>(false);
//   const [editLoading, setEditLoading] = useState<boolean>(false);
//   const [editedTodos, setEditedTodo] = useState<TaskType[]>([]);
//   const [searchParams, setSearchParams] = useSearchParams();
//   const [selectedTodo, setSelectedTodo] = useState<TaskType>();
//   const dispatch = useAppDispatch();
  
//   const [filteredTodosObject, setfilteredTodosObject] =
//     useState<FilterTodoObjectType>({
//       pageSize: parseInt(searchParams.get("pageSize") ?? "10"),
//       currentPage: parseInt(searchParams.get("currentPage") ?? "1"),
//       date: searchParams.get("date") ?? undefined,
//     });

//   const columns: ColumnsType<TaskType> = [
//     {
//       title: "Task No.",
//       key: "task_no",
//       width: "3rem",
//       render: (_, __, index: number) =>
//         filteredTodosObject.currentPage! * filteredTodosObject.pageSize! -
//         filteredTodosObject.pageSize! +
//         index +
//         1,
//     },
//     {
//       title: <div style={{ textAlign: "center" }}>Title</div>,
//       key: "title",
//       width: "6rem",
//       render: (record: TaskType) => {
//         return (
//           <>
//             <Row justify={"center"}>
//               <Col>
//                 {record.title}
//               </Col>
//             </Row>
//           </>
//         );
//       },
//     },
//     {
//       title: <div style={{ textAlign: "center" }}>Description</div>,
//       key: "description",
//       width: "9rem",
//       render: (record: TaskType) => {
//         return (
//           <>
//             <Row justify={"center"}>
//               <Col>
//                 {record?.description!.length > 50 ? record.description?.slice(0, 50) + "..." : record.description}
//               </Col>
//             </Row>
//           </>
//         );
//       },
//     },
//     {
//       title: <div style={{ textAlign: "center" }}>Date</div>,
//       key: "date",
//       width: "5rem",
//       render: (record: TaskType) => {
//         return (
//           <>
//             <Row justify={"center"}>
//               <Col>
//                 {record.date? record.date : "No date found"}
//               </Col>
//             </Row>
//           </>
//         );
//       },
//     },
//     {
//       title: <div style={{ textAlign: "center" }}>Action</div>,
//       key: "action",
//       width: "4rem",
//       render: (record: TaskType) => (
//         <Row justify={"space-evenly"}>
//           <Col>
//             <Button
//               onClick={() => {
//                 setSelectedTodo(record);
//                 setModalOpen(true);
//               }}
//               icon={<EditOutlined />}
//               style={{
//                 backgroundColor: "white",
//                 boxShadow: "0 4px 10px rgba(0, 0, 0, 0.1)",
//                 color: "black",
//               }}
//             />
//           </Col>
//           <Col>
//             <Button
//               onClick={() =>{
//                 setSelectedTodo(record)
//                 setDeletePromptModalOpen(true);
//               }}
//               icon={<DeleteOutlined />}
//               style={{
//                 backgroundColor: "white",
//                 boxShadow: "0 4px 10px rgba(0, 0, 0, 0.1)",
//                 color: "black",
//               }}
//             />
//           </Col>
//         </Row>
//       ),
//     },
//   ];
//   const handleDeleteRow = (id: string) => {
//     setEditLoading(true);
//     dispatch(DeleteTaskByIdThunk({ id: id }))
//       .then((data) => {
//         if (data.payload.success) {
//           filterTodos();
//           setDeletePromptModalOpen(false);
//         }
//       }).finally(() => {
//         setEditLoading(false);
//       });
//   };

//   const handleEditTodo = (editedTodos: TaskType, id: string) => {

//     if (!editedTodos) {
//       return message.error("Task not found in the database!");
//     }

//     const payloadBody = {
//       id: editedTodos.id,
//       title: editedTodos.title,
//       description: editedTodos.description,
//       date: editedTodos?.date!,
//     };
//     setEditLoading(true);
//     dispatch(EditTaskThunk({ body: payloadBody, id: id }))
//       .then((data) => {
//         if (data.payload.success) {
//           filterTodos();
//           setEditedTodo([]);
//           setModalOpen(false);
//         }
//         setEditLoading(false);
//       })
//       .catch((error) => {
//         setEditLoading(false);
//         message.error("Failed to edit todo:", error);
//       });
//   };

//   const handlePageChange = (pageNumber: number, pageSize: number) => {
//     setfilteredTodosObject((prevObj) => ({
//       ...prevObj,
//       currentPage: pageNumber,
//       pageSize,
//     }));
//   };
//   const filterTodos = () => {
//     let searchParams: SearchParamsType = {
//       pageSize: `${filteredTodosObject.pageSize}`,
//       currentPage: `${filteredTodosObject.currentPage}`,
//     };
//     if (filteredTodosObject.date) {
//       searchParams.date = filteredTodosObject.date;
//     }
//     setSearchParams(searchParams);

//     dispatch(
//       GetTaskList({
//         params: {
//           pageSize: filteredTodosObject.pageSize,
//           pageNumber: filteredTodosObject.currentPage ,
//           date: filteredTodosObject.date,
//         },
//       })
//     )
//       .then((data) => {
//         if (data.payload) {
//           setEditedTodo(data.payload.data);
//         }
//       })
//       .catch((error) => {
//         message.error("Failed to fetch todo list:", error);
//       });
//   };
//   useEffect(() => {
//     filterTodos();
//   }, [filteredTodosObject]);
//   return (
//     <>
//       <>
//         <EditTodoModal
//           modalOpen={modalOpen}
//           setModalOpen={setModalOpen}
//           selectedTodo={selectedTodo}
//           loading={editLoading}
//           handleEditTodo={handleEditTodo}
//         />
//         <Modal
//           title="Delete Todo"
//           open={deletePromptModalOpen}
//           footer={false}
//           onCancel={() => setDeletePromptModalOpen(false)}
//           closable={!editLoading}
//           maskClosable={!editLoading}
//           confirmLoading={true}
//           centered
//           width={380}
//         >
//           <Row>
//             <Col>
//               <p>Are you sure you want to delete this todo?</p>
//             </Col>
//           </Row>
//           <Row justify="end" gutter={[16,36]}>
//             <Col span={6}>
//               <Button style={{backgroundColor:'white', color:'black'}} onClick={() => setDeletePromptModalOpen(false)}>Cancel</Button>
//             </Col>
//             <Button
//               type="primary"
//               danger
//               style={{
//                 boxShadow: "0 4px 10px rgba(0, 0, 0, 0.1)",
//                 color: "white",
//               }}
//               onClick={() => handleDeleteRow(selectedTodo?.id!)}
//               loading={editLoading}
//             >
//               Delete
//             </Button>
//           </Row>
//         </Modal>
//       </>
//       <div className="content">
//         <Row align="middle">
//           <Col xs={{ span: 8 }}>
//             <h3>Todo List ({todos.total_count})</h3>
//           </Col>
//           <Col xs={{ span: 16 }}>
//             <Row justify="end" align="middle">
//               <Pagination
//                 showSizeChanger
//                 current={filteredTodosObject.currentPage}
//                 onChange={handlePageChange}
//                 total={todos.total_count}
//                 pageSize={filteredTodosObject.pageSize}
//                 itemRender={(currentPage, type, originalElement) => {
//                   if (type === "page") {
//                     return (
//                       <a
//                         style={
//                           currentPage === filteredTodosObject.currentPage
//                             ? {
//                                 backgroundColor: "whiter",
//                                 color: "black",
//                                 border: "1px solid black",
//                                 borderRadius: "4px",
//                               }
//                             : {}
//                         }
//                       >
//                         {currentPage}
//                       </a>
//                     );
//                   }
//                   return originalElement;
//                 }}
//               />
//             </Row>
//           </Col>
//         </Row>
//         <Row justify="end">
//           <DatePicker
//             onChange={(date) => {
//               setfilteredTodosObject((prevObj) => ({
//                 ...prevObj,
//                 date: date?.format("DD-MM-YYYY"),
//               }));
//             }}
//             placeholder="Select Date"
//             format="DD-MM-YYYY"
//           />
//         </Row>
//         <Row className="table-content" style={{ marginTop: "1.5rem" }}>
//           <Col xs={{ span: 24 }}>
//             <Table
//               loading={todos.isLoading}
//               columns={columns}
//               dataSource={editedTodos}
//               scroll={{ x: "100%" }}
//               rowKey="id"
//               bordered={true}
//               pagination={false}
//               sticky
//               //   style={{ backgroundColor: darkMode ? '#333' : 'white', color: darkMode ? 'white' : 'black' }}
//             />
//           </Col>
//         </Row>
//       </div>
//     </>
//   );
// }
